        ...
        for i in range(range_xfirst, range_xlast+1):
            for j in range(range_yfirst, range_ylast+1):
                if i < 0 or i >= board.width or j < 0 or j >= board.height:
                    continue
                for game_object in board.game_objects:
                    if game_object.position.x == i and game_object.position.y == j:
                        if game_object.type == "DiamondGameObject" or game_object.type == "TeleportGameObject": 
                            cek = False
                ...
                if isTeleport:
                    for game_object in board.game_objects:
                        if game_object.type == "TeleportGameObject" and game_object.position.x != x and game_object.position.y != y:
                            temp_x = game_object.position.x
                            temp_y = game_object.position.y
                            print("Teleport to: ", game_object.position.x,game_object.position.y)
                            break
                    Position(temp_x, temp_y)
                    count = self.count(board_bot, board, Position(x, y))
                    if count > 5:
                        return (y, x)
                    else:
                        return self.DIAMOND(board_bot, board)
        return (y, x)
